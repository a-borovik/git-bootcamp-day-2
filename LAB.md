...
Ветку пушил на `main`. При первом пуше ругнулся, что нет удаленная и локальная ветки не связны, необходимо было запустить команду  git push --set-upstream origin main
Проверка публичности репозитория была проверена открытием через вкладку инкогнито в браузере.
